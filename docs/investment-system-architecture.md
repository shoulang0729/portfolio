...
- 構成（順序確定）: **1 資産サマリ / 2 マクロ / 3 ヘルスチェック / 4 PERアナリシス / 5 メンター / 6 トリガー＆アクション / 7 来週**。`<section id="sN">`・`<b>N</b>`・上部 `.nav` の番号/ラベル/順序は常にこの並びで一致させる。
  - **§1 資産サマリ（#310→拡充・厳守）**: 生成時は下記の DON'T / DO を厳密に守る（過去、生成器が `.total` を復活させる回帰あり）。
    - **❌ §1 に入れてはいけない要素**:
      - `<div class="total">…</div>`（¥570M 等の**総資産/運用資産総額の大きな金額**）— stats バーと重複。**いかなる総額数値も §1 に出さない**。
      - 現預金・暗号資産の**金額の内訳行**（例 `現金・預金 ¥75M …`）— Exposure と重複。
      - キャッシュ比率の値の**後ろの括弧書き**（例 `（運用資産の14.3%）`『（投資用 ¥55M ÷ …）』等の算式/補足）。
    - **✅ §1 の構成（この順・この要素だけ）**:
      ```html
      <div class="card">
        <div class="cashline"><span class="k">キャッシュ比率</span><span class="v">NN%</span></div>
        <table class="perf">…運用資産の期間騰落（1週/1月/3月/1年）…</table>
        <div class="movers">…今週の主役 Top3（▲値上がり / ▼重し・保有＋ウォッチ）…</div>
        <div class="insight">…Claudeのインサイト…</div>
        <div class="note">…概算/母数の注記…</div>
      </div>
      ```
    - cashline は**ラベル「キャッシュ比率」＋値の % のみ**（括弧・金額・内訳を付けない）。期間騰落・主役は保有＋ウォッチの概算で `.note` に明記。
  ...
  - **本文ヘッダー**: 大見出し `<h1>Briefing</h1>` は**載せない**（アプリのタブで「Briefing」は明示済み・重複）。日付＋スタンス行（`.date`）は残す。アプリ側ツールバーも号タイトル/日付ラベルは出さず、生成ボタンのみのミニマル表示。
