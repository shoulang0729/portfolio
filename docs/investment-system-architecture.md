...
- 構成（順序確定）: **1 サマリ / 2 トレンド / 3 マクロ / 4 ヘルスチェック / 5 PERアナリシス / 6 メンター / 7 トリガー＆アクション**。`<section id="sN">`・`<b>N</b>`・上部 `.nav` の番号/ラベル/順序は常にこの並びで一致させる。
  - **§1 サマリ**: 資産総額カード（最上段）＋まとめ箇条書きカード（タイトルなし）の2枚構成。
    - **資産総額カード（§1 最上段・必須）**: アプリの stats バーは廃止。資産総額はここが唯一の表示場所。
      ```html
      <div class="card">
        <div class="total">¥XXX.XM<small>資産総額（Money Forward実値・YYYY-MM-DD）</small></div>
        <div class="cashline" style="align-items:flex-end;">
          <div style="display:flex;flex-direction:column;gap:3px;"><span class="k">現預金</span><span class="v">¥XX.XM</span></div>
          <div style="display:flex;flex-direction:column;gap:3px;"><span class="k">投資用</span><span class="v">≈¥XX.XM</span></div>
          <div style="display:flex;flex-direction:column;gap:3px;"><span class="k">比率</span><span class="v">≈XX%</span></div>
        </div>
      </div>
      ```
    - **まとめカード（§1 第2カード）**: タイトル（📋 今週のまとめ 等）は**不要**。資産行は**出さない**（上の資産総額カードに集約済み）。地合い・PER・メンター・ヘルス・アクションのみ `<div class="sum-row">` で列挙。
