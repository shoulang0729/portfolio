...
- 構成（順序確定）: **1 資産サマリ / 2 マクロ / 3 ヘルスチェック / 4 PERアナリシス / 5 メンター / 6 トリガー＆アクション / 7 来週**。`<section id="sN">`・`<b>N</b>`・上部 `.nav` の番号/ラベル/順序は常にこの並びで一致させる。
  - **§1 資産サマリ（#310→#319→確定・厳守）**: **資産の金額・比率は一切 §1 に出さない**（総資産=stats バー／キャッシュ=stats バーの「投資用キャッシュ」／現預金内訳=Exposure に集約済み）。生成器が `.total`・キャッシュ行を復活させる回帰が複数回あったため DON'T を厳守。
    - **❌ §1 に入れてはいけない要素**:
    ...
    - **✅ §1 の構成（この順・この要素だけ）**:
      ```html
      <div class="card">
        <table class="perf">…今週の地合い（列＝月/火/水/木/金の5列。非営業日は「休」。値は質的表現でも可）…</table>
      ...
      ```
    - 「今週の地合い」は**必ず月〜金の5列**（非営業日は「休」と明示）。主役は保有＋ウォッチの概算で `.note` に明記。
