...
**🟠 要確認（Toshio・軽微）**
- **Issue#538『Briefing トレンド表：モバイルではみ出し・バッジ重なり』が OPEN のまま滞留**（最終更新 2026-07-06・ラベルなし）。前サイクル（7/08 pm-queue）で **根治完了・監視終了** と記録済み（アプリ側 PR#539 ＋ 生成側 spec/SKILL 是正の二重防御）。**根治済みなら close してよい案件**＝実機（スマホ）で overflow 解消を目視確認のうえ #538 をクローズ。Mulmo は Issue を勝手にクローズしないため Toshio 判断待ち。※4日経過で「長期滞留」ではないが、根治宣言済み Issue の open 残りとして surfacing。
