...
> ✅ **調査完了（2026-06-25・#496 クローズ）**＝日本株データは取得可能と判明。個別ファンダ=Yahoo quoteSummary（PER/EV/EBITDA/PBR/粗利率/ROE 等・forwardPEのみ空）、セクター中央値=TOPIX-17 ETF proxy の **PER のみ**（17業種ETF全取得可）。意味ある比較が出る（例 ファストリ PE53.4 vs 小売24.8＝割高 / 小松15.7 vs 機械27.3＝割安）。詳細は本doc下部「調査結果」。**本実装＝#497 に起票済**。
