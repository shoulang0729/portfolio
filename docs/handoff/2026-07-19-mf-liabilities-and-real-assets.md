...
2. **不動産が自宅1件しかMFに存在しない。** MF total を分解すると `財務資産561M + 自宅88M` にちょうど一致。**収益物件（***REMOVED***・***REMOVED***）とその借入はMFに未登録** → MFの純資産すら実態を過小評価。
...
|---|---|---|---|---|---|---|
| ***REMOVED*** | 自宅・無収益 | ***REMOVED*** | イエシルAI査定/低 | 残 *****REMOVED***円** | **1.175% 変動・元金均等** | 三菱UFJ(■■■) |
| ***REMOVED*** | 収益・賃貸 | 未確定 | 管理会社AI査定/低 | 未確定(2025金利変更あり=変動) | 未確定 | 岐阜(母経由書類) |
| ***REMOVED*** | 収益・賃貸 | 未確定 | 管理会社AI査定/低 | 未確定 | 未確定 | 未確定 |

自宅ローン: 当初***REMOVED***(2019/2借入・35年)、完済2054/2、次回利息分***REMOVED***/月（年約35.3万）。
...
- **`mf-import-config.json` 拡張**:
  ...
  - `liabilityAccountMap`（口座名→物件/用途タグ）を新設。例: MUFG■■■→`自宅`。
