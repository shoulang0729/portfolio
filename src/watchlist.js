...
function onWatchlistSearch(eventOrQuery) {
  ...
  const version = ++_wlSearchVersion;
  _wlSearchTimer = setTimeout(() => searchTicker(q, version), 500);
}

async function searchTicker(q, version) {
  const dropdown = document.getElementById('wl-search-dropdown');
  ...
  if (version !== _wlSearchVersion) return;
...
export function renderWatchlist() {
  ...
  const rows = sorted.map(item => {
    ...
    const eExchange = escapeHTML(item.exchange || '');
    return `<tr>
      <td data-col="symbol" class="sl-sym">${eSym}<span class="sl-inline-name">${eName}</span></td>
      <td class="wl-market-cell"><span class="wl-type-badge">${eExchange}</span></td>
