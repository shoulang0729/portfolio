...
//
// 数値モデル（#594・docs/wealth-networth-spec.md・2026-07-21 確定）:
//   総資産（グロス） = mfNetWorth（MF そのまま）
//   運用資産         = 金融合計（現金+株+投信+債券+FX+暗号+保険+年金）− 生活資金¥20M
//                      ※ ポイント・その他・不動産は除外
//   不動産補正       = realEstateMf − realAssetsTotal（realEstateMf 未取得時は 0 で degrade）
//   純資産           = mfNetWorth − 不動産補正 − liabilitiesTotal
//   ★運用アロケーション（Risk Exposure 等）の分母に実物・負債・不動産を混ぜない（#577 §B）
// ══════════════════════════════════════════════════════════════
 ...
 * @type {{asOf?:string, totals?:{imported?:number, mfNetWorth?:number, liabilitiesTotal?:number, realAssetsTotal?:number, realEstateMf?:number}, holdings?:Array<{cat:string,cur?:string,value:number}>, liabilities?:MfLiability[]}|null}
 */
let _mf = null;

/**
 * mf-history.json の最新行（pension/insurance の補完源）。
 * wealth.js が loadHistory() 後に setMfHistoryLatest() で注入する。
 * @type {{pension?:number, insurance?:number}|null}
 */
let _historyLatest = null;

/**
 * mf-history の最新レコードを注入する（年金・保険の補完に使用）。
 * wealth.js の loadHistory() 成功後に呼ぶ。
 * @param {{pension?:number, insurance?:number}} latest
 */
export function setMfHistoryLatest(latest) {
  _historyLatest = latest || null;
}
...
/**
 * 総資産・現金・暗号資産・キャッシュ比率の集計。
 * #594: 運用資産・純資産を spec の派生式で計算する。未ロードなら null。
 *
 * ★運用アロケーション（Risk Exposure 等）の分母に使う値（imported/cash/crypto/securities/
 *   dryPowder/cashRatio）は従来どおり不変（実物・負債・不動産を混ぜない・#577 §B）。
 */
export function getMfTotals() {
  if (!_mf || !_mf.holdings) return null;
  const imported = (_mf.totals && _mf.totals.imported) || _sum(() => true);
  // MF 総資産（グロス）= mfNetWorth そのまま（#594 spec: 総資産定義）
  ...
  // #594 P2: 運用資産 = 金融合計（holdings から取れるカテゴリ）+ 年金/保険（mf-history 最新）− 生活資金
  // holdings は現金・株・投信・債券・FX・暗号を含む。年金・保険は MF BS 内訳にのみあり holdings には無い。
  const pensionFromHistory = (_historyLatest && Number(_historyLatest.pension)) || 0;
  const insuranceFromHistory = (_historyLatest && Number(_historyLatest.insurance)) || 0;
  const financialFromHoldings = imported; // 現金+株+投信+債券+FX+暗号 = imported（ポイント/不動産/その他は holdings に入れない方針）
  const investableAssets =
    typeof t.liabilitiesTotal === 'number'
      ? Math.max(0, financialFromHoldings + pensionFromHistory + insuranceFromHistory - EMERGENCY_FUND)
      : undefined;

  // #594 P2: 純資産 = mfNetWorth − 不動産補正 − 負債
  // 不動産補正 = realEstateMf − realAssetsTotal（realEstateMf 未取得時は 0 で degrade）
  let pureNetWorth = undefined;
  if (typeof t.liabilitiesTotal === 'number') {
    ...
    pureNetWorth = netWorth - realEstateCorrection - t.liabilitiesTotal;
  }
