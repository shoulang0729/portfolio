...
export function getMfTotals() {
  ...
  const t = _mf.totals || {};
  // 運用資産 = imported（holdings の金融資産合計）− 生活資金
  // ★holdings は ポイント・その他・不動産を含まない（scrape の include.categories 設計）。
  // 年金・保険は holdings に無いため現時点では除外（データ源配線は別途対応）。
  const investableAssets = typeof t.liabilitiesTotal === 'number' ? Math.max(0, imported - EMERGENCY_FUND) : undefined;
  ...
  } else if (typeof t.liabilitiesTotal === 'number') {
    const reAdj =
      typeof t.realEstateMf === 'number'
        ? t.realEstateMf - (typeof t.realAssetsTotal === 'number' ? t.realAssetsTotal : 0)
        : 0;
    netWorthComputed = netWorth - reAdj - t.liabilitiesTotal;
  }
  return {
    netWorth,
    imported,
