 ...
 * @param {RequestInit} [opts={}] fetch オプション（既存 signal がある場合はマージ）
 */
 ...
 * @param {number} [opts.retryDelay=2000] - リトライ前の待機ms（指数バックオフのベース値）
...
export async function batchWithRetry(items, fn, opts = {}) {
  ...
  for (let attempt = 0; attempt < maxRetries && failedIndices.length > 0; attempt++) {
    const backoff = retryDelay * Math.pow(2, attempt) + Math.random() * 500;
    await sleep(backoff);
    await Promise.all(failedIndices.map(async idx => {
      results[idx] = await fn(items[idx]);
    }));
    failedIndices = failedIndices.filter(idx => isFailed(results[idx], idx));
  }
