...
async function netWorthCardHTML() {
  ...
  // #594: 総資産 = mfNetWorth（MFそのまま）。旧: imported + realAssetsTotal
  ...
  return `<div class="card we-card we-nwcard">
    ...
    <div class="we-nw-note">実物資産＝AI査定(HowMa)×掛目（都市部1.0／地方0.65）の採用値。<b>実物資産・負債は運用アロケーション（Risk / リバランス）の分母に含めない。</b>総資産はMF表示値（${escapeHTML(fmtYen(mf.netWorth))}）をそのまま採用（#594）。</div>
  </div>`;
}
