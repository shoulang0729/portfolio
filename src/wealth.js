...
import { loadMfHoldings, getMfTotals, getMfLiabilities, setMfHistoryLatest } from './networth.js';
 ...
 * ロード後に最新行を setMfHistoryLatest で配線し、運用資産計算（#594 P2）に使う。
 */
async function loadHistory() {
  ...
  if (_data.series.length > 0) {
    const latest = _data.series[_data.series.length - 1];
    setMfHistoryLatest({ pension: latest.pension, insurance: latest.insurance });
  }
  return _data;
}
...
/**
 * ネットワース3層カード（#577・handoff 2026-07-19 §B）。
 * 「運用資産 / 実物資産 / 負債」＋計算純資産を data/mf-holdings.json（v5 totals）から表示する。
 * ★実物資産・負債は文脈表示のみ＝運用アロケーション（Risk Exposure 等）には一切影響しない。
 * 負債未取得（v4 形）の間はプレースホルダを出す。実物資産は data/real-assets/ 着地までは
 * realAssetsTotal=0 ＝「未投入」表示。金額は目隠し（_eye）に追従する。
 */
async function netWorthCardHTML() {
  ...
  const nw = typeof mf.netWorthV6 === 'number' ? mf.netWorthV6 : mf.netWorth - mf.liabilitiesTotal;
  const oku = (n) => `${(n / 1e8).toFixed(2)}億`;
  // 表示用: 運用資産（年金+保険追加済み）vs MF総資産
  const invAssets = typeof mf.investmentAssets === 'number' ? mf.investmentAssets : null;
  const real = typeof mf.realAssetsTotal === 'number' ? mf.realAssetsTotal : 0;
  const realEstateMf = typeof mf.realEstateMf === 'number' ? mf.realEstateMf : null;
  ...
  const realEstateMfCell = realEstateMf !== null ? escapeHTML(fmtYen(realEstateMf)) : '<span class="we-nw-na">—</span>';
  const realEstateCorrection =
    realEstateMf !== null && real > 0 ? realEstateMf - real : null;
  const grossTxt = escapeHTML(fmtYen(mf.netWorth));
  return `<div class="card we-card we-nwcard">
    ...
    <div class="we-table-wrap"><table class="t we-table we-nw">
      <tbody>
        ${invAssets !== null ? `<tr><td><span class="we-op"> </span>運用資産</td><td>${escapeHTML(fmtYen(invAssets))}</td></tr>` : ''}
        <tr><td><span class="we-op"> </span>総資産（MF）</td><td>${grossTxt}</td></tr>
        ${realEstateMf !== null ? `<tr class="we-nw-sub"><td><span class="we-op">−</span>不動産補正（MF評価−実価）</td><td>${realEstateCorrection !== null && realEstateCorrection > 0 ? `−${escapeHTML(fmtYen(realEstateCorrection))}` : realEstateMfCell}</td></tr>` : ''}
    ...
    <div class="we-nw-note">純資産 = MF総資産 − 不動産補正 − 負債。実物資産＝AI査定(HowMa)×掛目の採用値。<b>実物資産・負債は運用アロケーション（Risk / リバランス）の分母に含めない。</b></div>
  </div>`;
}
