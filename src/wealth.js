...
/**
 * ネットワース3層カード（#594 spec 2026-07-21）。
 * 総資産=mfNetWorth／純資産=mfNetWorth−不動産補正−負債のウォーターフォール表示。
...
async function netWorthCardHTML() {
  ...
  // 不動産補正 = realEstateMf（MF評価）− realAssetsTotal（現実評価・haircut後）
  ...
  const correctionRow =
    realEstateCorrection !== 0
      ? `<tr class="we-nw-liab"><td><span class="we-op">−</span>不動産補正（MF評価→現実評価）</td><td>−${escapeHTML(fmtYen(realEstateCorrection))}</td></tr>`
      : '';
  return `<div class="card we-card we-nwcard">
    ...
    <div class="we-nw-note"><b>実物資産・負債は運用アロケーション（Risk / リバランス）の分母に含めない。</b>不動産補正＝AI査定(HowMa)×掛目（都市部1.0／地方0.65）との差額。</div>
  </div>`;
}
