...
/** data/mf-history.json をロード（キャッシュ）。最新レコードの年金・保険を networth.js に注入（#594 P2）。 */
async function loadHistory() {
  ...
  if (_data.series.length > 0) {
    ...
    setMfHistoryLatest({
      pension: typeof latest.pension === 'number' ? latest.pension : null,
      insurance: typeof latest.insurance === 'number' ? latest.insurance : null,
    });
  }
  return _data;
}
...
/**
 * ネットワース3層カード（#594 数値モデル再定義・2026-07-21）。
 * 総資産=MF総資産(mfNetWorth) / 純資産=MF総資産−不動産補正−負債 の新モデルで表示する。
 * ★実物資産・負債は文脈表示のみ＝運用アロケーション（Risk Exposure 等）には一切影響しない（#577 §B）。
...
async function netWorthCardHTML() {
  ...
  // #594 数値モデル: 総資産 = mfNetWorth（MFそのまま）
  const gross = mf.netWorth;
  // 純資産 = mfNetWorth − 不動産補正 − 負債（netWorthV6 が新式・fallback は旧 gross−liab）
  const nw = typeof mf.netWorthV6 === 'number' ? mf.netWorthV6 : gross - mf.liabilitiesTotal;
  const oku = (n) => `${(n / 1e8).toFixed(2)}億`;
  const real = mf.realAssetsTotal || 0;
  const realEstateMf = mf.realEstateMf;
  const realEstateCorrection =
    typeof realEstateMf === 'number' && real > 0 ? realEstateMf - real : null;
  ...
  const liabRows = liabs
    ...
    .join('');
  const realEstateRow =
    realEstateCorrection !== null
      ? `<tr class="we-nw-sub"><td><span class="we-op"> </span>不動産補正（MF評価→実態）</td><td>−${escapeHTML(fmtYen(Math.abs(realEstateCorrection)))}</td></tr>`
      : '';
  return `<div class="card we-card we-nwcard">
    ...
    <div class="we-table-wrap"><table class="t we-table we-nw">
      <tbody>
        <tr class="we-nw-subtotal"><td><span class="we-op"> </span>総資産（MF）</td><td>${escapeHTML(fmtYen(gross))}</td></tr>
        ${realEstateRow}
    ...
    <div class="we-nw-note"><b>実物資産・負債は運用アロケーション（Risk / リバランス）の分母に含めない。</b>不動産補正＝MF評価−AI査定(HowMa)×掛目の差分（P1実装後に反映）。</div>
  </div>`;
}
