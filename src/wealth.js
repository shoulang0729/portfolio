...
/**
 * ネットワース3層カード（#594 数値モデル再定義・表示は P3 で別途）。
 * ウォーターフォール: MF総資産 −[不動産の盛り分] −[負債] ＝ 純資産（手取り）
 * ★実物資産・負債は文脈表示のみ＝運用アロケーション（Risk Exposure 等）には一切影響しない。
 * 負債未取得（v4 形）の間はプレースホルダを出す。金額は目隠し（_eye）に追従する。
 */
async function netWorthCardHTML() {
  ...
  const totalAssets = mf.netWorth;
  const real = mf.realAssetsTotal || 0;
  const realMf = mf.realEstateMf;
  const reCorrection = typeof realMf === 'number' ? realMf - real : 0;
  const nw = mf.pureNetWorth ?? totalAssets - mf.liabilitiesTotal;
  const oku = (n) => `${(n / 1e8).toFixed(2)}億`;
  const operating = mf.operatingAssets || mf.imported;
  const pctOp = totalAssets > 0 ? (operating / totalAssets) * 100 : 0;
  const pctRe = totalAssets > 0 ? (real / totalAssets) * 100 : 0;
  ...
  const liabRows = liabs
    ...
    .join('');
  const reCorrectionCell =
    reCorrection !== 0
      ? `<tr><td><span class="we-op">−</span>不動産補正（MF評価−採用値）</td><td>−${escapeHTML(fmtYen(Math.abs(reCorrection)))}</td></tr>`
      : '';
  const realInfoCell =
    real > 0
      ? `<tr class="we-nw-sub"><td>　実物資産採用値</td><td>${escapeHTML(fmtYen(real))}</td></tr>`
      : '';
  const bar =
    real > 0
      ? `<div class="we-nw-bar"><span class="we-nw-seg s-un" style="width:${pctOp.toFixed(1)}%"></span><span class="we-nw-seg s-re" style="width:${pctRe.toFixed(1)}%"></span></div>
      <div class="we-nw-legend"><span><i class="s-un"></i>運用 ${pctOp.toFixed(0)}%</span><span><i class="s-re"></i>実物 ${pctRe.toFixed(0)}%</span><span class="we-nw-gross">総資産(MF) ${escapeHTML(fmtYen(totalAssets))}</span></div>`
      : '';
  return `<div class="card we-card we-nwcard">
    ...
    <div class="we-table-wrap"><table class="t we-table we-nw">
      <tbody>
        <tr><td><span class="we-op"> </span>総資産（MF）</td><td>${escapeHTML(fmtYen(totalAssets))}</td></tr>
        ${reCorrectionCell}
        ${realInfoCell}
    ...
    <div class="we-nw-note">実物資産＝AI査定(HowMa)×掛目（都市部1.0／地方0.65）の採用値。<b>実物資産・負債は運用アロケーション（Risk / リバランス）の分母に含めない。</b>運用資産(#594)＝金融合計 − 生活資金¥20M（年金・保険込み・不動産除く）。</div>
  </div>`;
}
