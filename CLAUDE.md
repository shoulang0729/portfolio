...
│   ├── history.js          # 資産推移記録＋D3グラフ（★未統合：index.html に未ロード）
│   ├── ai-system-prompt.js # AI相談ペルソナ（★無効化中：index.html でコメントアウト）
│   ├── ai-tab.js           # AI相談タブ（★無効化中：index.html でコメントアウト）
...
│   ├── 05-ai-tab.css       # AI相談タブ全般（現在ほぼ未使用）
...
- **現在 index.html には読み込まれていない**。統合するには `<script>` タグ追加と `switchTab` に `'history'` ケースを追加する
...
1. index.html の `<!-- <script src="src/history.js?v=...">` コメントを外す
2. index.html のタブバーに `<button data-tab="history" ...>資産推移</button>` を追加
3. `<section id="panel-history" class="tab-panel">` パネルを追加
4. `app.js` の `switchTab()` に `if (name === 'history') renderHistoryTab();` を追加
5. `data.js` の `refreshPrices()` 成功後に `recordTodayAsset()` を呼ぶ
