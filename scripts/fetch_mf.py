...
def attach_liabilities(c, doc, liab_rows, re_vals=None):
    ...
    netWorthComputed = mfNetWorth −（不動産補正）− liabilitiesTotal（#594 spec 2026-07-21）。
    不動産補正 = realEstateMf − realAssetsTotal（realEstateMf 未取得時は 0＝補正なし degrade）。
    既存 mfNetWorth（MF 画面の資産グロス生値）は比較用にそのまま残す。
    re_vals: scrape_real_estate() の {fileId: 評価額}（None は未取得）。
    """
    ...
    re_mf = int(round(sum(re_vals.values()))) if re_vals else None
    re_adj = (re_mf - ra) if isinstance(re_mf, int) else 0
    mf_net = doc["totals"].get("mfNetWorth", doc["totals"].get("imported", 0))
    doc["totals"]["liabilitiesTotal"] = lt
    doc["totals"]["realAssetsTotal"] = ra
    if isinstance(re_mf, int):
        doc["totals"]["realEstateMf"] = re_mf
    doc["totals"]["netWorthComputed"] = mf_net - re_adj - lt
    return doc
...
def do_run(c):
    ...
    try:
        ...
        doc = attach_liabilities(c, doc, liab, re_vals)  # verify 後＝資産チェックサムに影響しない（#577）＝完全版（KV送信用）
